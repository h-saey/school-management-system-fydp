...
export function ViewMarks() {
  ...
  const [stats, setStats] = useState({ average: 0, highest: 0, lowest: 0, totalExams: 0 });
  ...
  const calculateStats = (marksData: any[]) => {
    ...
    const percentages = marksData.map(m => (m.marksObtained / m.totalMarks) * 100);
  ...
  const handleDownloadReport = () => {
    toast.success('Report card download functionality would be implemented here');
    // In real implementation, generate PDF report
  };

  if (loading) {
    return <div className="flex items-center justify-center h-64"><div className="text-gray-600">Loading marks...</div></div>;
  }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">View your examination performance and grades</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center gap-3 mb-2">
              <FileText className="w-6 h-6 text-orange-500" aria-hidden="true" />
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label htmlFor="term-filter" className="block text-sm text-gray-700 mb-2">Term</label>
              <select
                id="term-filter"
                value={selectedTerm}
            ...
            <div>
              <label htmlFor="subject-filter" className="block text-sm text-gray-700 mb-2">Subject</label>
              <select
                id="subject-filter"
            ...
            <div>
              <label htmlFor="exam-filter" className="block text-sm text-gray-700 mb-2">Exam Type</label>
              <select
                id="exam-filter"
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-sm text-gray-700">Exam Type</th>
                  <th className="px-6 py-3 text-left text-sm text-gray-700">Term</th>
                  <th className="px-6 py-3 text-center text-sm text-gray-700">Marks</th>
                  <th className="px-6 py-3 text-center text-sm text-gray-700">Percentage</th>
                  <th className="px-6 py-3 text-center text-sm text-gray-700">Grade</th>
                </tr>
              </thead>
                  ...
                  filteredMarks.map((mark, index) => {
                    ...
                    return (
                      <tr key={index} className="hover:bg-gray-50">
                        <td className="px-6 py-4 text-gray-900">{mark.subject}</td>
                        <td className="px-6 py-4 text-gray-600">{mark.examType}</td>
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className={`px-3 py-1 rounded-full text-sm ${getGradeColor(mark.grade)}`}>
                            {mark.grade}
                          </span>
                        </td>
                      </tr>
              ...
              return (
                <div key={subject} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">{subjectMarks.length} exam(s)</p>
                  </div>
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <p className="text-sm text-gray-600">Average</p>
                      <p className="text-gray-900 font-medium">{Math.round(avgPercentage * 10) / 10}%</p>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-sm ${getGradeColor(avgGrade)}`}>
                      {avgGrade}
                    </span>
                  </div>
                </div>
