...
namespace SMS_Backend.Controllers
{
    ...
    public class MarkController : ControllerBase
    {
        ...
        public MarkController(ApplicationDbContext context, AuditLogService audit)
        {
            _context = context;
            _audit = audit;
        }
        ...
        public async Task<IActionResult> GetAllMarks(
            [FromQuery] int? studentId, [FromQuery] string? subject)
        {
            var query = _context.Marks
                .Include(m => m.Student).Include(m => m.Teacher).AsQueryable();
            if (studentId.HasValue) query = query.Where(m => m.StudentId == studentId.Value);
            if (!string.IsNullOrEmpty(subject)) query = query.Where(m => m.Subject == subject);

            return Ok(await query.Select(m => new
            {
                m.MarksId,
                m.Subject,
                m.Exam,
        ...
        // GET: api/marks/my  — Student sees own
        [HttpGet("my")]
        [Authorize(Roles = "Student")]
        public async Task<IActionResult> GetMyMarks([FromQuery] string? subject)
        {
        ...
        // GET: api/marks/student/{studentId}  — Parent or Teacher
        [HttpGet("student/{studentId}")]
        [Authorize(Roles = "Admin,Teacher,Parent")]
        public async Task<IActionResult> GetStudentMarks(int studentId)
        {
            ...
            return Ok(await _context.Marks
                .Where(m => m.StudentId == studentId)
                .Select(m => new { m.MarksId, m.Subject, m.Exam, m.MarksObtained, m.TotalMarks, m.Percentage })
                .ToListAsync());
        }
        ...
        public async Task<IActionResult> AddMarks([FromBody] AddMarksDto dto)
        {
            ...
            await _audit.LogAsync(GetUserId(), AuditLogService.Actions.AddMarks,
                $"Added marks for StudentId={dto.StudentId} Subject={dto.Subject} Exam={dto.Exam} Score={dto.MarksObtained}/{dto.TotalMarks}");

            return CreatedAtAction(nameof(GetAllMarks), new { },
                new { message = "Marks added.", mark.MarksId });
        }
        ...
        {
            var mark = await _context.Marks.FindAsync(id);
            if (mark == null) return NotFound(new { message = "Mark not found." });
            if (dto.MarksObtained > mark.TotalMarks)
                return BadRequest(new { message = "Marks obtained cannot exceed total." });

            var old = mark.MarksObtained;
            mark.MarksObtained = dto.MarksObtained;
            if (dto.TotalMarks.HasValue) mark.TotalMarks = dto.TotalMarks.Value;

            await _context.SaveChangesAsync();

            await _audit.LogAsync(GetUserId(), AuditLogService.Actions.UpdateMarks,
                $"MarksId={id} updated {old} → {dto.MarksObtained}");

            return Ok(new { message = "Marks updated." });
        }
        ...
        public async Task<IActionResult> DeleteMarks(int id)
        {
            ...
            await _audit.LogAsync(GetUserId(), AuditLogService.Actions.DeleteMarks,
                $"Deleted MarksId={id}");
