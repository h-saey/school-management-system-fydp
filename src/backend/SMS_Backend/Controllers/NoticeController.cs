...
namespace SMS_Backend.Controllers
{
    ...
    public class NoticeController : ControllerBase
    {
        ...
        public NoticeController(ApplicationDbContext context) => _context = context;
        ...
        public async Task<IActionResult> GetNotices([FromQuery] NoticeAudience? audience)
        {
            ...
            var query = _context.Notices.Include(n => n.PostedBy)
                            .Where(n => n.IsActive).AsQueryable();
            ...
            var result = await query.OrderByDescending(n => n.PostedAt)
                .Select(n => new
                {
                    n.NoticeId,
                    n.Title,
        ...
        {
            var notice = await _context.Notices.Include(n => n.PostedBy)
        ...
        public async Task<IActionResult> PostNotice([FromBody] PostNoticeDto dto)
        {
            ...
            if (dto.Audience == NoticeAudience.ClassSpecific && string.IsNullOrWhiteSpace(dto.TargetClass))
                return BadRequest(new { message = "TargetClass required for ClassSpecific notices." });
            ...
            return CreatedAtAction(nameof(GetNotice), new { id = notice.NoticeId },
                new { message = "Notice posted.", notice.NoticeId });
        }
        ...
        public async Task<IActionResult> UpdateNotice(int id, [FromBody] UpdateNoticeDto dto)
        {
            ...
            if (notice == null) return NotFound(new { message = "Notice not found." });
            if (GetCurrentUserRole() == "Teacher" && notice.PostedByUserId != GetCurrentUserId())
                return Forbid();
            ...
            if (dto.Type.HasValue) notice.Type = dto.Type.Value;
            if (dto.Priority.HasValue) notice.Priority = dto.Priority.Value;
            if (dto.TargetClass != null) notice.TargetClass = dto.TargetClass;

            await _context.SaveChangesAsync();
            return Ok(new { message = "Notice updated." });
        }
    ...
    public class PostNoticeDto
    {
        ...
        public NoticeType Type { get; set; } = NoticeType.Academic;
