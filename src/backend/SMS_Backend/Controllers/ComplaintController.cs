...
namespace SMS_Backend.Controllers
{
    ...
    public class ComplaintController : ControllerBase
    {
        ...
        public ComplaintController(ApplicationDbContext context, AuditLogService audit)
        {
            _context = context;
            _audit = audit;
        }
        ...
        public async Task<IActionResult> GetComplaints()
        {
            ...
            IQueryable<Complaint> query = _context.Complaints
                .Include(c => c.SubmittedBy).Include(c => c.AssignedTo);

            if (role == "Student" || role == "Parent")
                query = query.Where(c => c.SubmittedByUserId == currentId);
            ...
            return Ok(await query.Select(c => new
            {
                ...
                AssignedTo = c.AssignedTo == null
                    ? null : new { c.AssignedTo.Username, c.AssignedTo.Role }
            }).ToListAsync());
        }
        ...
        public async Task<IActionResult> GetComplaint(int id)
        {
            ...
            var role = GetUserRole();
            if ((role == "Student" || role == "Parent")
                && complaint.SubmittedByUserId != GetUserId())
                return Forbid();
        ...
        public async Task<IActionResult> SubmitComplaint([FromBody] SubmitComplaintDto dto)
        {
            ...
            await _audit.LogAsync(GetUserId(), AuditLogService.Actions.SubmitComplaint,
                $"Submitted complaint Category={dto.Category}");

            return CreatedAtAction(nameof(GetComplaint), new { id = complaint.ComplaintId },
                new { message = "Complaint submitted.", complaint.ComplaintId });
        }
        ...
        public async Task<IActionResult> UpdateStatus(int id, [FromBody] UpdateComplaintStatusDto dto)
        {
            ...
            complaint.Status = dto.Status;
            if (!string.IsNullOrWhiteSpace(dto.Remarks)) complaint.Remarks = dto.Remarks;
            if (dto.Status == ComplaintStatus.Closed || dto.Status == ComplaintStatus.Resolved)
                complaint.DateClosed = DateTime.UtcNow;

            await _context.SaveChangesAsync();

            await _audit.LogAsync(GetUserId(), AuditLogService.Actions.UpdateComplaint,
                $"ComplaintId={id} status {oldStatus} → {dto.Status}");

            return Ok(new { message = "Status updated.", complaint.Status });
        }
        ...
        public async Task<IActionResult> AssignComplaint(int id, [FromBody] AssignComplaintDto dto)
        {
            ...
            var valid = await _context.Users.AnyAsync(u =>
                u.UserId == dto.AssignedToUserId &&
                (u.Role == UserRole.Teacher || u.Role == UserRole.Admin));

            if (!valid) return BadRequest(new { message = "Assignee must be Teacher or Admin." });

            complaint.AssignedToUserId = dto.AssignedToUserId;
            complaint.Status = ComplaintStatus.UnderReview;
            await _context.SaveChangesAsync();

            await _audit.LogAsync(GetUserId(), AuditLogService.Actions.AssignComplaint,
                $"ComplaintId={id} assigned to UserId={dto.AssignedToUserId}");

            return Ok(new { message = "Complaint assigned." });
        }
