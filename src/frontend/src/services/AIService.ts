...
// Student/Parent: studentId=0 → backend uses JWT identity
// Teacher/Admin:  pass real studentId
...
// Student uses this — no params needed
export const getMyRisk = () => get<RiskPrediction>(`${API_BASE}/ai/my-risk`);

// Parent uses this — backend finds their child automatically
export const getChildRisk = () =>
  get<RiskPrediction>(`${API_BASE}/ai/child-risk`);

// Teacher uses this — returns all students they teach
export const getMyStudentsRisks = () =>
  get<AllRisksResponse>(`${API_BASE}/ai/my-students-risks`);

// Admin uses this — all students with filters
export const getAllRisks = (params?: {
  ...
  if (params?.minMarks !== undefined)
    q.set("minMarks", String(params.minMarks));
  if (params?.maxMarks !== undefined)
    q.set("maxMarks", String(params.maxMarks));
  const qs = q.toString();
