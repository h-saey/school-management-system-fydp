...
import { Award, FileText, Calendar, Download, Trophy, Star, Medal } from 'lucide-react';

export function StudentPortfolio() {
  const [activeTab, setActiveTab] = useState<'academics' | 'attendance' | 'certificates' | 'achievements'>('academics');
  ...
  const certificates = [
    { name: 'Science Olympiad Gold Medal', issuedBy: 'Dr. Singh', date: '2025-11-15', type: 'Academic' },
    { name: 'Inter-School Debate Winner', issuedBy: 'Mrs. Sharma', date: '2025-10-20', type: 'Co-curricular' },
    { name: 'Perfect Attendance Award', issuedBy: 'Principal', date: '2025-09-01', type: 'Achievement' }
  ];

  const achievements = [
    { title: '100% Attendance', icon: Calendar, color: 'bg-green-500', date: 'Aug-Nov 2025' },
    { title: 'Top 3 in Mathematics', icon: Star, color: 'bg-yellow-500', date: 'Mid-Term 2025' },
    { title: 'Science Fair Winner', icon: Trophy, color: 'bg-blue-500', date: 'Oct 2025' },
    { title: 'Best Speaker Award', icon: Medal, color: 'bg-purple-500', date: 'Sep 2025' }
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-600">Comprehensive record of your academic journey</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-2">
        <div className="flex gap-2">
          {[
            { id: 'academics' as const, label: 'Academic Records', icon: FileText },
            { id: 'attendance' as const, label: 'Attendance Report', icon: Calendar },
            { id: 'certificates' as const, label: 'Certificates', icon: Award },
            { id: 'achievements' as const, label: 'Achievements & Activities', icon: Trophy }
          ].map((tab) => {
            const Icon = tab.icon;
            return (
              <button
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="p-4 bg-yellow-50 border-l-4 border-yellow-500 rounded">
            <p className="text-gray-700">
              <strong>Note:</strong> Maintain at least 75% attendance to be eligible for examinations.
            </p>
          </div>
        </div>
      )}
              ...
              {achievements.map((achievement, index) => {
                ...
                return (
                  <div key={index} className="p-6 border border-gray-200 rounded-lg text-center">
                    <div className={`${achievement.color} w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4`}>
