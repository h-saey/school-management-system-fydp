...
import { AIPanel, UserRoleForAI } from "./AIPanel";

interface AIWidgetProps {
  userRole: UserRoleForAI;
}

export function AIWidget({ userRole }: AIWidgetProps) {
  ...
  return (
    <>
      ...
      >
        {open ? (
          <X style={{ color: "white", width: 22, height: 22 }} />
        ) : (
          <Brain style={{ color: "white", width: 22, height: 22 }} />
        )}
      </button>
      ...
      {open && <AIPanel onClose={() => setOpen(false)} userRole={userRole} />}
    </>
  );
}
// import React, { useState } from "react";
