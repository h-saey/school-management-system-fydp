...
export function ManageParents() {
  ...
  const fetchAll = async () => {
    try {
      setFetching(true);
      const res = await fetch(`${API_BASE}/parent`, { headers: authH() });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.message);
      setParents(data);

      const [pu, st] = await Promise.all([
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      error(err.message ?? "Failed to create parent");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateParent(editParent.parentId, { relation: editRelation });
  ...
  // Only show Parent-role users that don't have a profile yet
  const existingParentUserEmails = new Set(parents.map((p) => p.email));
  const availableUsers = parentUsers.filter(
    (u) => !existingParentUserEmails.has(u.email),
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600">
            Link parent accounts to student profiles
          </p>
        </div>
        <button
          ...
          >
            ...
            <div className="md:col-span-2">
              <label className="block text-gray-700 mb-1 text-sm">
                Parent User Account <span className="text-red-500">*</span>
                <span className="text-gray-400 ml-2 font-normal">
                  (Parent-role accounts without a profile)
                </span>
              </label>
              {availableUsers.length === 0 ? (
                <div className="px-4 py-3 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-700 text-sm">
                  No available Parent accounts. Create one in{" "}
                  <strong>Manage Users</strong> with Parent role first.
                </div>
              ) : (
                <select
                  required
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-40 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="bg-gray-50 rounded-lg p-3 mb-4 text-sm text-gray-700">
              ...
              ({editParent.linkedStudent.rollNumber})
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="p-6 border-b flex items-center gap-3">
          ...
          <h2 className="text-gray-900">Parents ({filtered.length})</h2>
        </div>
        {fetching ? (
          <div className="p-8 text-center text-gray-500">Loading...</div>
        ) : filtered.length === 0 ? (
          <div className="p-8 text-center text-gray-400">
            {searchTerm ? "No parents match." : "No parents yet."}
          </div>
        ) : (
