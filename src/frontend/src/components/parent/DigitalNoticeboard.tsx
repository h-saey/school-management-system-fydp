...
import { getNotices, type NoticeRecord } from "../../services/parentApi";

// Map backend type string/number to icon & color
function noticeIcon(type: string) {
  switch (type?.toLowerCase()) {
    ...
    case "holiday":
      return { icon: PartyPopper, color: "bg-green-500" };
    case "event":
      return { icon: Calendar, color: "bg-purple-500" };
    case "academic":
      return { icon: Info, color: "bg-indigo-500" };
    default:
...
export function DigitalNoticeboard() {
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-500">Loading noticeboard…</div>
      </div>
    );
  ...
  // Derive unique type filters from live data
  const typeOptions = [
    "All",
    ...Array.from(new Set(notices.map((n) => n.type))),
  ];
  const filterOptions =
    typeOptions.length > 1
      ? typeOptions
      : ["All", "Exam", "Fee", "Holiday", "Event", "Academic"];

  const filteredNotices =
    filter === "All"
      ? notices
      : notices.filter((n) => n.type?.toLowerCase() === filter.toLowerCase());
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="text-right text-sm text-gray-600">
                        <p>{new Date(notice.postedAt).toLocaleDateString()}</p>
                        <p className="text-xs text-gray-500 mt-1">
                          By: {notice.postedBy?.username ?? "Admin"}
                        </p>
                      </div>
                    </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-gray-700 text-sm">
              Check the digital noticeboard regularly for important updates.
              High priority notices require immediate attention. You can also
              enable notifications in your account settings to receive alerts
              for new notices.
            </p>
          </div>
        </div>
